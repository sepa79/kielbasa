...
const char * plantStage[] = {
    TXT_PLANT_STAGE_NONE,
    TXT_PLANT_STAGE_PLOW_TASK_ASSIGNED,
    TXT_PLANT_STAGE_PLOWED,
...
static void _sowPlant(){
    if(_currentPlant == 0){
        updateStatusBarError(TXT_CONFIRM_ERROR);
        return;
    }
    // // must be plowed
    ...
    // byte idx = plants[_currentPlant].taskDscIdx;   // TXT_PL_ARRAY
    // const char * idx = plants[_currentPlant].taskDscIdx;   // TXT_PL_ARRAY
    // "Field 2, Potatoes"
    // sprintf(newTask.desc, "%s %u, %s", TXT_TASK_DSC_FARMLAND_FIELD, _currentField+1, plantType[_currentPlant]);  // TXT_PL_ARRAY
    newTask.codeRef   = &sowFieldTask;
    // newTask.nameIdx   = TXT_TASK_FARMLAND_FARM;   // TXT_PL_ARRAY
...
static void _reapPlant(){
    ...
    // byte idx = plants[fields[_currentField].plantId].taskDscIdx;   // TXT_PL_ARRAY
    // "Field 2, Potatoes"
    // sprintf(newTask.desc, "%s %u, %s", TXT_CACHE[LTXT_IDX_TASK_DSC_FARMLAND_FIELD], _currentField+1, TXT[idx]);  // TXT_PL_ARRAY
    newTask.codeRef   = &reapFieldTask;
    // newTask.nameIdx   = TXT_PL_TASK_FARMLAND_REAP;   // TXT_PL_ARRAY
...
static void _plowField() {
    ...
    // "Field 2"
    sprintf(newTask.desc, "%s %u", TXT_TASK_DSC_FARMLAND_FIELD, _currentField + 1);
    newTask.codeRef   = &plowFieldTask;
    // newTask.nameIdx   = TXT_PL_TASK_FARMLAND_PLOW;   // TXT_PL_ARRAY
    newTask.params[0] = _currentField;
    newTask.params[1] = 0;
    newTask.params[2] = 0;
    newTask.params[3] = 0;
...
static void _showField(){
    cwin_putat_string_raw(&cw, FARM_CTX_MENU_X+4, 1, TXT_FARM_FIELD_TXT, VCOL_MED_GREY);
    byte str[2];
...
static void _showConfirmBox(){
    // Confirm:
    cwin_putat_string_raw(&cw, FARM_CTX_MENU_X+4, 2, TXT_CONFIRM_TXT, VCOL_MED_GREY);
    // yes/no
    if(_confirm){
        cwin_putat_string_raw(&cw, FARM_CTX_MENU_X+4, 3, TXT_CONFIRM_YES, VCOL_LT_GREY);
    } else {
        cwin_putat_string_raw(&cw, FARM_CTX_MENU_X+4, 3, TXT_CONFIRM_NO, VCOL_LT_GREY);
    }
    // arrows
...
static void _showOptionPlow(){
    // cwin_clear(&cd);
    cwin_putat_string_raw(&cw, FARM_CTX_MENU_X, FARM_CTX_MENU_Y, TXT_MENU_FARMLAND_PLOW, VCOL_DARK_GREY);
    cwin_write_string_raw(&ht, TXT_DESC_FARMLAND_PLOW);

    _showField();
    _showConfirmBox();
}
static void _showOptionSow(){
    cwin_putat_string_raw(&cw, FARM_CTX_MENU_X, FARM_CTX_MENU_Y, TXT_MENU_FARMLAND_SOW, VCOL_DARK_GREY);
    cwin_write_string_raw(&ht, TXT_DESC_FARMLAND_SOW);

    _showField();

    // Growing:
    cwin_putat_string_raw(&cw, FARM_CTX_MENU_X+4, 2, TXT_FARM_PLANT_TXT, VCOL_MED_GREY);
    // plantname
    cwin_putat_string_raw(&cw, FARM_CTX_MENU_X+4, 3, plantType[_currentPlant], VCOL_LT_GREY);     // TXT_PL_ARRAY
    // arrows
    cwin_putat_string_raw(&cw, FARM_CTX_MENU_X+2, 3, "\x1f", VCOL_MED_GREY);
    cwin_putat_string_raw(&cw, FARM_CTX_MENU_X+14, 3, "$", VCOL_MED_GREY);
}
static void _showOptionReap(){
    ...
    cwin_putat_string_raw(&cw, FARM_CTX_MENU_X, FARM_CTX_MENU_Y, TXT_MENU_FARMLAND_REAP, VCOL_DARK_GREY);
    cwin_write_string_raw(&ht, TXT_DESC_FARMLAND_REAP);
}
...
static void _displayFieldList(){
    ...
    cwin_putat_string_raw(&cw, COL_OFFSET_FIELDLIST, ROW_OFFSET_FIELDLIST, TXT_FIELD_LIST_HEADER, VCOL_YELLOW);
    ...
    for(byte i=0;i<FIELDS_COUNT;i++){
        ...
        // field size
        cwin_putat_string_raw(&cw, COL_OFFSET_FIELDLIST, ROW_OFFSET_FIELDLIST+1+i*3, fieldSize[fields[i].area-1], col);   // TXT_PL_ARRAY
        // plant name
        char plantId = fields[i].plantId;
        cwin_putat_string_raw(&cw, COL_OFFSET_FIELDLIST+1, ROW_OFFSET_FIELDLIST+2+i*3, plantType[plantId], col);   // TXT_PL_ARRAY

        // stage
        char stage = fields[i].stage;
        cwin_putat_string_raw(&cw, COL_OFFSET_FIELDLIST+13, ROW_OFFSET_FIELDLIST+1+i*3, plantStage[stage], col);   // TXT_PL_ARRAY
...
const struct MenuOption FARMLAND_MENU[] = {
    // options to select
    { TXT_MENU_SELECT, KEY_RETURN, SCREEN_SPLIT_MC_TXT, UI_F+UI_HIDE, &_selectField, 0, 23, 4},
    // standard navigation
    { TXT_MENU_A, 'a', SCREEN_SPLIT_MC_TXT, UI_L+UI_HIDE, &_previousPlant, 0, 1, 2},
    { TXT_MENU_D, 'd', SCREEN_SPLIT_MC_TXT, UI_R+UI_HIDE, &_nextPlant, 0, 1, 2},
    { TXT_MENU_W, 'w', SCREEN_SPLIT_MC_TXT, UI_U+UI_HIDE, &_previousField, 0, 7, 2},
    { TXT_MENU_S, 's', SCREEN_SPLIT_MC_TXT, UI_D+UI_HIDE, &_nextField, 0, 9, 2},
    { TXT_EXIT_TO_MAP, KEY_ARROW_LEFT, SCREEN_TRANSITION, UI_LF, &showMenu, MENU_BANK_MAP_VILLIAGE_1, 32, 12},
    END_MENU_CHOICES
};
