...
def write_texts_c_file( template_fullpath, text_array, index_array, lang ):
    out = []
...
def create_pl_lang_files( dot_c_template, dot_h_template, config ):
    # textsPL.c
    txt_array = generate_c_file_text_arrays( config, "pl", encode_charset )
    index_arrays = generate_c_file_pl_index_arrays( config, "pl" )
    write_texts_c_file( dot_c_template, txt_array, index_arrays, "pl" )
    # textsPL.h
    create_h_file_text_arrays( dot_h_template, "pl" )

# master function for creating common.h file
def create_common_h_file( fullpath, template, config ):
    index_array = generate_common_h_index_array( config )
    write_common_h_file( fullpath, template, index_array )
